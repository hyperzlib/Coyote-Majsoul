        ...
        for (const api of apis) {
            try {
                ...
                if (res.version && compareVersion(versionInfo.version, res.version)) {
                    ...
                    logger.info(`检测到新版本：${res.version}，更新内容：\n${res.description}\n`);

                    if (releaseFile) {
                        logger.info(`下载地址：${api.release.replace('{repo}', res.repo).replace('{version}', res.version).replace('{file}', releaseFile)}`);
                    }
                    
                    return true;
                }
            } catch (e: any) {
                logger.debug('Cannot check update: ' + e.message, e);
            }
        }
    } catch (e: any) {
        logger.debug('Cannot check update: ' + e.message, e);
